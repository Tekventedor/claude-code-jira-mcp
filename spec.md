...
**Total**: 45 s · 30 fps · 1350 frames · 1920 × 1080 · FlowHunt palette · Inter + JetBrains Mono.
...
| 3 | s3-arch | Architecture | 450–690 | 8.0 s | Pipeline diagram: prompt → Claude → MCP → Jira |
| 4 | s4-install | Install | 690–870 | 6.0 s | The `claude mcp add` one-liner + OAuth flash |
| 5 | s5-snapshot | Snapshot magic | 870–1110 | 8.0 s | KAN-4 issue lands in Jira, side-by-side with JQL |
| 6 | s6-cta | CTA | 1110–1350 | 8.0 s | FlowHunt logo + blog title + button + URL |
...
- `output.duration * fps == 1350`. Verify on every build.
